...
const Dashboard = () => {
  ...
  const handleSaveArticle = async (e) => {
    ...
    try {
      ...
      if (res.success) {
        alert(editingArticleId ? 'সংবাদটি সফলভাবে আপডেট করা হয়েছে!' : 'সংবাদটি সফলভাবে তৈরি করা হয়েছে!');
        resetEditorForm();
        setActiveTab('overview');
      } else {
        alert(res.message || 'সংবাদটি সংরক্ষণ করতে ব্যর্থ হয়েছে।');
      }
    } catch (err) {
      console.error(err);
    }
  ...
  const handleDeleteArticle = async (id) => {
    ...
    try {
      ...
      if (res.success) {
        ...
        alert('আর্টিকেল সফলভাবে মুছে ফেলা হয়েছে।');
        fetchArticlesList();
      }
    } catch (err) {
      console.error(err);
                  ...
                  <div className="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/40 p-6 rounded-2xl shadow-xs space-y-4">
                    ...
                    <div>
                      ...
                      {articleFeaturedImage && (
                        <img 
                          src={articleFeaturedImage.startsWith('/') ? `http://localhost:5000${articleFeaturedImage}` : articleFeaturedImage} 
                          alt="preview" 
