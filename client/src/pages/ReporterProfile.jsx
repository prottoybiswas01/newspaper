...
const ReporterProfile = () => {
  ...
  return (
    <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      {/* Back button */}
      <Link to="/" className="inline-flex items-center text-xs font-bold text-gray-500 hover:text-gray-900 dark:hover:text-neutral-200 mb-6">
        <ArrowLeft className="h-4 w-4 mr-1" />
        <span>হোমপেজে ফিরে যান</span>
      </Link>

      {/* Reporter Header Info */}
      <div className="bg-white dark:bg-[#121212] border border-gray-200 dark:border-neutral-800 rounded-2xl p-6 shadow-xs mb-10 flex flex-col md:flex-row items-center gap-6">
        <img 
          src={reporter.avatar || `https://api.dicebear.com/7.x/adventurer/svg?seed=${reporter.name}`} 
          alt={reporter.name} 
          className="h-24 w-24 rounded-full border border-gray-300 dark:border-neutral-700 object-cover bg-gray-100 dark:bg-neutral-800" 
        />
        ...
        {/* Total stats */}
        <div className="border-t md:border-t-0 md:border-l border-gray-200 dark:border-neutral-800 pt-4 md:pt-0 md:pl-8 text-center shrink-0">
          <div className="flex items-center space-x-2 text-gray-500 dark:text-neutral-400 justify-center mb-1">
            <FileText className="h-5 w-5 text-gray-400" />
            <span className="text-xs font-bold uppercase tracking-wider">প্রকাশিত সংবাদ</span>
          </div>
          <span className="text-3xl font-black text-gray-900 dark:text-neutral-100">{reporter.articleCount}</span>
        </div>
      </div>

      {/* Articles list */}
      <h2 className="text-lg font-black text-gray-950 dark:text-white mb-6 border-b border-gray-200 dark:border-neutral-800 pb-2">
        {reporter.name}-এর প্রকাশিত খবরসমূহ
      </h2>

      {articles.length === 0 ? (
        <div className="text-center py-12 bg-white dark:bg-[#121212] border rounded-xl border-gray-200 dark:border-neutral-800">
          <p className="text-gray-500 dark:text-neutral-400 italic">বর্তমানে লেখকের কোনো প্রকাশিত সংবাদ নেই।</p>
        </div>
      ) : (
            ...
            <div key={art._id} className="group bg-white dark:bg-[#121212] rounded-xl overflow-hidden border border-gray-200 dark:border-neutral-800 shadow-xs hover:shadow-lg transition-all duration-300">
              <Link to={`/article/${art.slug}`}>
                ...
                <div className="p-4 space-y-2">
                  ...
                  <div className="flex items-center text-[10px] text-gray-400 dark:text-neutral-500 pt-2 font-semibold">
